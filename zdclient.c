...
extern uint8_t      eap_response_idekp[];   /*EAP RESPON/INDENTITY KEEPALIVE报文E*/
...
//    printf("PackType: %d\n", pType);
    switch(pType){
        ...
        case EAP_REQUEST_IDENTITY_KEEP_ALIVE:
            ...
            *(eap_response_idekp + 14 + 5) = eap_head->eap_id;
...
//    eap_response_md5ch = malloc (14 + 4 + 6 + 16 + username_length + 46);
    ...
    memset(eap_response_idekp, 0, 14+9+16+33);
    data_index = 0;
    memcpy(eap_response_idekp, eapol_eth_header, 14);
    data_index += 14;
    memcpy(eap_response_idekp + data_index, eap_resp_idkp_head,9);
    data_index += 25;
    memcpy(eap_response_idekp + data_index, local_info_tailer,33); //No client version string
